...
public partial class SgDataGrid<TItem> : ComponentBase, IAsyncDisposable where TItem : notnull
{
    ...
    /// <summary>Expands all tree nodes recursively in tree mode.</summary>
    ...
    /// <summary>Collapses all tree nodes in tree mode.</summary>
    ...
    /// <summary>Gets the set of currently selected items.</summary>
    ...
    /// <summary>Exports the current grid state (filters, sort, pagination, column layout, query rules, row highlight rules) as a <see cref="SgGridState"/> object.</summary>
    ...
    /// <summary>Exports the current grid state as a JSON string.</summary>
    public string ExportStateJson() =>
        JsonSerializer.Serialize(ExportState(), StateJsonOptions);

    /// <summary>Imports a previously exported grid state, restoring filters, sort, pagination, column layout, and highlight rules.</summary>
    /// <param name="state">The grid state to import.</param>
    /// <returns><c>true</c> if the state was successfully imported; otherwise <c>false</c>.</returns>
    ...
    /// <summary>Imports grid state from a JSON string produced by <see cref="ExportStateJson"/>.</summary>
    /// <param name="json">The JSON string containing the grid state.</param>
    /// <returns><c>true</c> if the state was successfully imported; otherwise <c>false</c>.</returns>
    ...
    /// <summary>Resets the grid state to its default values, clearing filters, sort, pagination, and column layout changes.</summary>
    ...
    /// <summary>Applies query rules to the grid, replacing any existing query rules.</summary>
    /// <param name="rules">The query rules to apply, or null to clear.</param>
    ...
    /// <summary>Clears all active query rules from the grid.</summary>
    ...
    /// <summary>Forces the grid to re-render and invalidate all cached rows.</summary>
    public void Refresh()
    {
        EnsureAutoGeneratedColumns();
        _itemsVersion++;
    ...
    /// <summary>Applies row highlight rules to the grid, replacing any existing rules.</summary>
    /// <param name="rules">The highlight rules to apply, or null to clear.</param>
    ...
    /// <summary>Clears all active row highlight rules.</summary>
...
private static object? ConvertFromString(string? text, Type type)
    ...
    /// <summary>Expands all collapsed groups in grouping mode.</summary>
    ...
    /// <summary>Collapses all groups in grouping mode.</summary>
    ...
    /// <summary>Handles keyboard events for grid navigation (arrow keys, home, end, page up/down, enter, escape).</summary>
    ...
    /// <summary>Clears all selected items.</summary>
    ...
    /// <summary>Releases module references, cancels pending operations, and disposes JS interop resources.</summary>
    ...
    /// <summary>Represents a node in the grouping hierarchy, containing grouped items and child groups.</summary>
    private sealed class GroupNode
    {
        /// <summary>Initializes a new <see cref="GroupNode"/> with the specified grouping properties.</summary>
        ...
        /// <summary>The unique path string identifying this group in the hierarchy.</summary>
        public string Path { get; }
        /// <summary>The column this group is based on.</summary>
        public SgDataGridColumn<TItem> Column { get; }
        /// <summary>The nesting depth of this group (0-based).</summary>
        public int Depth { get; }
        /// <summary>The display label for this group.</summary>
        public string Label { get; }
        /// <summary>The total number of items in this group, including children.</summary>
        ...
        /// <summary>Gets the items directly in this group (leaf groups only).</summary>
        public List<TItem> Items => _items ??= new List<TItem>(0);
        /// <summary>Gets the child group nodes (non-leaf groups only).</summary>
        public List<GroupNode> Children => _children ??= new List<GroupNode>(0);

        /// <summary>Sets the items for this leaf group.</summary>
        public void SetItems(List<TItem> items) => _items = items;
        /// <summary>Sets the child groups for this non-leaf group.</summary>
